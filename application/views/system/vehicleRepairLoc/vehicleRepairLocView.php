				...
				<div style="text-align: right;">
					<?php 
						...
						$this->session->userdata('sys_user_group_name') == "Manager"){
							...
							echo '<a type="button" href="'.base_url().'VehicleRepairLoc/create" class="btn text-dark btn-default btn-sm">
...
function loadData() {
	$.ajax({
		...
		url: API+"VehicleRepairLoc/fetch_all_join/",
		success: function(data, result){
			...
			$(function () {
				...
				$.each(data, function (i, item) {
					//console.log(item);
					var is_active_vhcl_repair_loc  ='';
					if(item.is_active_vhcl_repair_loc  == 1){
						is_active_vhcl_repair_loc  = '<span class="right badge badge-success">Active</span>';
					}
					else{
						is_active_vhcl_repair_loc  = '<span class="right badge badge-danger">Inactive</span>';
					}
					
					
					table.row.add([item.repair_loc_id,
					item.repair_loc_name,
					item.repair_loc_address, 					
					item.repair_loc_contact,
					is_active_vhcl_repair_loc ,
					...
					$(".editBtn").last().attr('href', '<?php echo base_url() ?>VehicleRepairLoc/edit/'+item.repair_loc_id);
					$(".viewBtn").last().attr('value',item.repair_loc_id);
				});
...
$(document).on('click','.viewBtn', function(){
	...
	$.ajax({
		...
		dataType: "json",
		contentType: 'application/json',
		url: API+"VehicleRepairLoc/fetch_single/?id="+repair_loc_id,
		success: function(data, result){
			console.log(data);
			
			Header = data[0].repair_loc_name;
			console.log(Header);
