	...
	$.ajax({
		...
		success: function(data, result){
			...
			var options={
				...
				autoclose: true,
				orientation: 'bottom',
				"defaultDate":new Date(data[0].stock_purchase_date)
			};
			
			date_input.datepicker(options);
			
			$('#stock_purchase_date').datepicker('setDate', data[0].stock_purchase_date);
			
			$('#item_name').val(data[0].item_name);		
			
						
			if(data[0].is_approved_stock == 1){
				$('#is_approved_stock').prop('checked', true);
			}
			
			if(data[0].is_allocated_stock == 1){
				$('#is_allocated_stock').prop('checked', true);
			}
			
			if(data[0].is_active_stock_purchase == 1){
			...
			$.each(data, function (i, item) {
				...
				if(item.item_id != null){
					...
					var rowHtml = '<tr class="mainItemSet itemRow" id="'+(numItems+1)+'">'+
								  ...
								  '</td>'+
								  '<td>'+
									'<input class="form-control" id="no_of_items" name="no_of_items" type="number" value="'+item.no_of_items+'" autocomplete="off">'+
								  '</td>'+
								  '<td>'+
									'<input type="number" class="form-control" id="item_cost" name="item_cost" value="'+item.item_cost+'" required>'+
								  '</td>'+
								  '<td>'+
				...
				if(item.sub_item_id != null){
					...
					var rowHtml = '<tr class="subItemSet itemRow" id="'+(numItems+1)+'">'+
								...
								setSubItem(elem, item.sub_item_id);							
