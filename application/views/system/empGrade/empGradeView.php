				...
				<div style="text-align: right;">
					<?php 
						...
						$this->session->userdata('sys_user_group_name') == "Manager"){
							...
							echo '<a type="button" href="'.base_url().'EmpGrade/create" class="btn text-dark btn-default btn-sm">
									<i class="nav-icon far fa-plus-square"></i> Add Grade
								</a>';
						}
...
function loadData() {
	...
	$.ajax({
		...
		url: API+"EmpGrade/fetch_all_join/",
		success: function(data, result){
			...
			$(function () {
				...
				$.each(data, function (i, item) {
					//console.log(item);
					var is_active_emp_grade  ='';
					if(item.is_active_emp_grade  == 1){
						is_active_emp_grade  = '<span class="right badge badge-success">Active</span>';
					}
					else{
						is_active_emp_grade  = '<span class="right badge badge-danger">Inactive</span>';
					}
					
					
					table.row.add([item.emp_grade_id,
					item.emp_grade_name,
					item.emp_grade_desc,
					is_active_emp_grade ,
					...
					$(".editBtn").last().attr('href', '<?php echo base_url() ?>EmpGrade/edit/'+item.emp_grade_id);
					$(".viewBtn").last().attr('value', item.emp_grade_id);
...
$(document).on('click','.viewBtn', function(){
	...
	emp_grade_id = $(this).attr('value');
	console.log(emp_grade_id);
	...
	$.ajax({
		...
		dataType: "json",
		contentType: 'application/json',
		url: API+"EmpGrade/fetch_single_join?id="+emp_grade_id,
		success: function(data, result){
			console.log(data);
			
			Header = 'Grade Id: '+data[0].emp_grade_id;
			console.log(Header);
