		...
		<div class="card card-primary">
			...
			<form>
				<div class="card-body ">
					<form>
						<div class="form-row">
							<input type="hidden" class="form-control" id="insuar_detail_id" required>
...
function loadData() {
	...
	$.ajax({
		...
		url: API+"vehicleInsurance/fetch_single/?id="+last_part,
		success: function(data, result){
			...
			$('#insuar_detail_id').val(data[0].insuar_detail_id);
			$('#insuar_comp_id').val(data[0].insuar_comp_id);
			$('#insuarance_number').val(data[0].insuarance_number);
			$('#insuar_type').val(data[0].insuar_type);
			$('#valid_from_date').val(data[0].valid_from_date);
			$('#valid_to_date').val(data[0].valid_to_date);
			$('#premimum_amount').val(data[0].premimum_amount);
						
			if(data[0].is_active_vhcl_ins_details == 1){
				$('#is_active_vhcl_ins_details').prop('checked', true);
			}
			...
			$.ajax({
				...
				success: function(data, result){
					...
					$.each(data, function(index, item) {
						...
						if(data[0].insuar_comp_id == item.insuar_comp_id){
							repair_loc_drp += '<option selected value="'+item.insuar_comp_id+'">'+item.insuar_comp_name+'</option>';
						}
						else{
...
$('#submit').click(function(e){
	...
	insuar_detail_id = $('#insuar_detail_id').val();
	...
	if(typeof insuar_detail_id !== 'undefined' && insuar_detail_id !== ''
	&& typeof insuar_comp_id !== 'undefined' && insuar_comp_id !== ''	
	...
	&& typeof vehicle_id !== 'undefined' && vehicle_id !== '' )
	{
		...
		formData.append('insuar_detail_id',insuar_detail_id);
		...
		$.ajax({
			...
			contentType: false,
			data: formData,				
			url: API+"vehicleInsurance/update/",
			success: function(data, result){
				...
				if(data.message == "Please Fill Required Fields!" || data.message == "Vehicle Insurance is being used by other modules at the moment!"){
					const notyf = new Notyf();
